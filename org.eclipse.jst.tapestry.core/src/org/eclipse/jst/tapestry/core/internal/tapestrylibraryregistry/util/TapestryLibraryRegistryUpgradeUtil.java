...
public class TapestryLibraryRegistryUpgradeUtil {
static String v1Tov2UpgradeURL = "http://www.eclipse.org/JSFxxxxxxxxxxxxxxxxxxxxxx"; //$NON-NLS-1$
	...
	private static TapestryLibraryRegistryUpgradeUtil INSTANCE;
		
	/**
	 * @return the stateful runtime singleton
	 */
	public static synchronized TapestryLibraryRegistryUpgradeUtil getInstance(){
		if (INSTANCE == null){
			INSTANCE = new TapestryLibraryRegistryUpgradeUtil();
		}
		return INSTANCE;
	}
	...
	public void upgradeRegistryIfNecessary(int expectedVersion) {
		...
		try
		{
			...
			if (curVersion < expectedVersion && curVersion != NO_VERSION)
			{
				...
				if (op.canExecute())
				{
					...
					catch (ExecutionException e)
					{
						...
						TapestryCorePlugin.log(IStatus.ERROR, "Error during loading JSF Library registry", e); //$NON-NLS-1$
						//TODO: flag failure in status
						upgradeStatus = 
							new UpgradeStatus(IStatus.ERROR, true, Messages.TapestryLibraryRegistryUpgradeUtil_Error); 
					}
				}
				else
				{
                    ...
                    upgradeStatus = 
                        new UpgradeStatus(IStatus.ERROR, false, Messages.TapestryLibraryRegistryUpgradeUtil_Error); 				}
			}
			else
			{
			...
			// should never happen since we control the URL's
	    	TapestryCorePlugin.log(IStatus.ERROR, "Error during loading Tapestry Library registry", e); //$NON-NLS-1$
		}
	}
	...
	{
		UpgradeOperation  op = new UpgradeOperation(Messages.TapestryLibraryRegistryUpgradeUtil_Operation); 
		switch(curVersion)
		{
			case 1:
				op.addVersionUpgrade(
						new MigrateV1toV2Operation(Messages.TapestryLibraryRegistryUpgradeUtil_v1Tov2Operation 
	...
	static void copyFile(String srcFileName, String destFileName) {
		...
		try {
			...
			TapestryCorePlugin.log(ioe, "Error during file copy"); //$NON-NLS-1$
		} finally {
			if (from != null)
				try {
					from.close();
				} catch (IOException e) {
					TapestryCorePlugin.log(e, "Error during file close"); //$NON-NLS-1$
				}
			if (to != null)
				try {
					to.close();
